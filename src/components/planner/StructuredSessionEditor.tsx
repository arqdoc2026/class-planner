    ...
    <section>
      ...
      <table className="w-full table-fixed border-collapse">
        ...
        <thead>
          ...
          <tr className="bg-[#d9d9d9] text-center font-bold"><th className="border border-black border-b-[3px] [border-bottom-style:double] p-2">Sesión</th><th className="border border-black border-b-[3px] [border-bottom-style:double] p-2">Resultados de aprendizaje</th><th className="border border-black p-2">Actividades de instrucción</th><th className="border border-black border-b-[3px] [border-bottom-style:double] p-2">Recursos</th></tr>
        </thead>
        {sessions.map((session, index) => (
          <tbody key={session.id || `session-${index}`}>
            <tr><td rowSpan={6} className="border border-black p-2 text-center align-top text-lg font-bold">{index + 1}<input type="date" value={session.plannedDate} onChange={(event) => change(index, { plannedDate: event.target.value })} className="mt-2 w-full bg-transparent text-[9px] font-normal outline-none" /></td><td rowSpan={6} className="border border-black p-0 align-top"><textarea value={session.learningResults} onChange={(event) => change(index, { learningResults: event.target.value })} className="min-h-52 w-full resize-y bg-transparent p-2 outline-none focus:bg-blue-50" /></td><th className="border border-black border-t-[3px] [border-top-style:double] bg-[#d0d0d0] p-1 text-left">Inicio</th><td rowSpan={6} className="border border-black border-t-[3px] [border-top-style:double] p-0 align-top"><textarea value={session.resources} onChange={(event) => change(index, { resources: event.target.value })} className="min-h-52 w-full resize-y bg-transparent p-2 outline-none focus:bg-blue-50" /></td></tr>
            <tr><td className="border border-black p-0"><textarea value={session.startActivity} onChange={(event) => change(index, { startActivity: event.target.value })} rows={2} className="w-full resize-y bg-transparent p-2 outline-none focus:bg-blue-50" /></td></tr>
            <tr><th className="border border-black bg-[#d0d0d0] p-1 text-left">Actividades de la clase</th></tr>
            <tr><td className="border border-black p-0"><textarea value={session.developmentActivity} onChange={(event) => change(index, { developmentActivity: event.target.value })} rows={2} className="w-full resize-y bg-transparent p-2 outline-none focus:bg-blue-50" /></td></tr>
            <tr><th className="border border-black bg-[#d0d0d0] p-1 text-left">Cierre</th></tr>
            <tr><td className="border border-black p-0"><textarea value={session.closingActivity} onChange={(event) => change(index, { closingActivity: event.target.value })} rows={2} className="w-full resize-y bg-transparent p-2 outline-none focus:bg-blue-50" /></td></tr>
